...
async def read_users(
    ...
    """检索所有用户"""
    return db.query(models.User).offset(skip*40).limit(40).all()
...
async def update_user(
    ...
    if new_data := data.dict(exclude_defaults=True):
        db.query(models.User).filter(
            models.User.id == user_id).update(new_data)
        db.commit()
    return {}
...
async def delete_user(
    ...
    """通过id删除用户"""
    db.query(models.Comment).filter(models.User.id == user_id).delete()
    db.commit()
    return {}
...
async def read_items(
    ...
    """检索所有项目"""
    return db.query(models.Item).offset(skip*40).limit(40).all()
...
async def update_item(
    ...
    if new_data := data.dict(exclude_defaults=True):
        db.query(models.Item).filter(
            models.Item.id == item_id).update(new_data)
        db.commit()
    return {}
